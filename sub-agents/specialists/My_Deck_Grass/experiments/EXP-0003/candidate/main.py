...
# Attacks whose printed Attack.damage does NOT match what the engine deals.
# Measured directly against the live engine (opponent Active HP before/after):
#
#   Myriad Leaf Shower (Teal Mask Ogerpon ex): printed 30, dealt 120/150/180.
#     The formula is 30 + 30 * (Energy PROVIDED to both Active Pokemon). Note
#     "provided", not "cards attached": one sample had the opponent on 2 provided
#     Energy from 1 card (their own Meganium's Wild Growth) and dealt 180, which
#     only the provided count explains. mon.energies is already the provided
#     count -- with Wild Growth out, one Basic {G} card shows as energies=[1,1] --
#     so no Wild Growth handling of our own is needed anywhere.
#   Cruel Arrow (Fezandipiti ex): printed 0, deals a flat 100 to any one of the
#     opponent's Pokemon.
#
# Everything downstream of damage (the lethal gate, attack choice, active_quality,
# ko_risk, attacker ranking) read the printed value, so the deck's early attacker
# was valued at 30 damage and its support ex at 0.
ATK_MYRIAD_LEAF_SHOWER = 120
ATK_CRUEL_ARROW = 183


def _live_attack_damage(atk, mon, opp_active=None) -> int:
    """Damage `atk` would actually deal from `mon`, not its printed value."""
    if atk is None:
        return 0
    if atk.attackId == ATK_MYRIAD_LEAF_SHOWER:
        mine = len(mon.energies) if mon is not None else 0
        theirs = len(opp_active.energies) if opp_active is not None else 0
        return 30 + 30 * (mine + theirs)
    if atk.attackId == ATK_CRUEL_ARROW:
        return 100
    return atk.damage


def _best_attack_index(options, indices, mon=None, opp_active=None):
    ...
    for i in indices:
        ...
        dmg = _live_attack_damage(atk, mon, opp_active)
        if dmg > best_dmg:
...
def _live_attacker_score(mon, opp_active=None) -> int:
    ...
    if mon is None:
        return -10**9
    dmg = _best_usable_damage(mon, opp_active)
...
def _choose_main(obs: Observation) -> list[int]:
    ...
    if OptionType.ATTACK in by_type and opp_active is not None:
        idx, dmg = _best_attack_index(options, by_type[OptionType.ATTACK], my_active, opp_active)
    ...
    if OptionType.ATTACK in by_type:
        idx, _ = _best_attack_index(options, by_type[OptionType.ATTACK], my_active, opp_active)
        return [idx]
...
def _choose_attack(obs: Observation) -> list[int]:
    ...
    if state is not None:
        ...
        my_active = me.active[0] if me.active else None
        opp_active = opp.active[0] if opp.active else None
    idx, _ = _best_attack_index(sel.option, list(range(len(sel.option))), my_active, opp_active)
    return [idx]
...
def _best_usable_damage(mon, opp_active=None) -> int:
    ...
    for aid in card.attacks:
        ...
        if a is not None and _can_pay(a.energies, mon.energies):
            best = max(best, _live_attack_damage(a, mon, opp_active))
    return best
...
def _eval_state(state, my_index: int) -> float:
    ...
    if my_active is not None:
        active_quality = _best_usable_damage(my_active, opp_active) * 2 + len(my_active.energies) * 5
    ...
    if my_active is not None and opp_active is not None:
        if _best_usable_damage(opp_active, my_active) >= my_active.hp:
