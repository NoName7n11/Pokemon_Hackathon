...
# Evolution lines, child -> pre-evolution, for judging whether a card in hand is live.
_EVOLVES_FROM = {651: 650, 652: 651, 918: 917, 710: 918}
_MEGANIUM = 710
_BOSSS_ORDERS = 1182


def _hand_card_id(opt, obs):
    """Card id behind a hand-area option; these carry cardId=None."""
    if opt.cardId:
        return opt.cardId
    state = obs.current
    if state is None or opt.index is None or opt.index < 0:
        return None
    owner = opt.playerIndex if opt.playerIndex is not None else state.yourIndex
    if not 0 <= owner < len(state.players):
        return None
    hand = getattr(state.players[owner], "hand", None) or []
    if opt.index < len(hand) and hand[opt.index] is not None:
        return getattr(hand[opt.index], "id", None)
    return None


def _keep_value(card_id, obs) -> int:
    """How much I want to KEEP this card. Discards take the lowest values first.

    Hand DISCARD options carry cardId=None, so the generic _option_card_power path scores
    every option 0 and the stable sort pays Ultra Ball's two-card cost with hand slots 0
    and 1. Observed: it discarded Meganium plus a Basic {G} while holding two spare Night
    Stretchers, and discarded Boss's Orders while holding a Bayleef.

    Meganium ranks highest because Wild Growth doubles every Basic {G}, which is what makes
    Jungle Dump cost two cards instead of four -- pitching it to pay for a search is the
    single worst discard available in this deck.
    """
    if card_id is None:
        return 0
    ...
    board = [m.id for m in mons if m is not None]
    hand = [getattr(c, "id", None) for c in (getattr(me, "hand", None) or [])]

    if card_id == _MEGANIUM:
        value = 400 if _MEGANIUM in board else 1000   # the Energy engine
    elif card.cardType == CardType.POKEMON:
        pre = _EVOLVES_FROM.get(card_id)
        if pre is not None:
            value = 800 if (pre in board or pre in hand) else 200   # live vs dead evolution
        elif card.basic:
            value = 550 if not board else 450                       # need a body on board
        else:
            value = 300
    elif card.cardType in (CardType.BASIC_ENERGY, CardType.SPECIAL_ENERGY):
        attached = sum(len(m.energies) for m in mons)
        value = 600 if attached < 4 else 300
    elif card_id == _BOSSS_ORDERS:
        value = 700                                                 # the prize-closing card
    else:
        value = 350                                                 # generic Trainer

    # Second and later copies in hand are the cheapest thing to pay with.
    if hand.count(card_id) > 1:
        value -= 250
    return value
...
def _choose_card(obs: Observation) -> list[int]:
    ...
    if sel.context in opponent_target_contexts:
        ...
        sel.context == SelectContext.DISCARD
        and obs.current is not None
        and all(options[i].area == AreaType.HAND for i in indices)
    ):
        # Paying a cost from hand (Ultra Ball). These options carry cardId=None, so the
        # generic path below scores them all 0 and pays with hand slots 0 and 1.
        ranked = sorted(indices, key=lambda i: _keep_value(_hand_card_id(options[i], obs), obs))
    else:
