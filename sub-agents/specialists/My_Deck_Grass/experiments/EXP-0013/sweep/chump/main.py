...
OGERPON, MEOWTH_EX, FEZANDIPITI_EX = 96, 1071, 140
MEGANIUM_ID = 710
CRUEL_ARROW_DAMAGE = 100


def _promotion_adjust(mon, state, opp_active) -> int:
    """Deck-role adjustment for FORCED PROMOTION only (TO_ACTIVE / SWITCH).

    `_live_attacker_score` ranks by raw HP and usable damage, which on this deck means the
    380 HP Mega Venusaur ex wins essentially every promotion regardless of role. Four
    deck-specific rules, all conditional on live board state:

      Ogerpon ex   -- its Tera clause prevents ALL attack damage while it is Benched, and
                      Teal Dance works from the Bench. Promoting it forfeits that for no
                      gain unless it can actually take the knockout now.
      Meowth ex    -- Tuck Tail deals 60 AND returns it plus attached cards to hand. That
                      dodges the 2-prize liability and lets Last-Ditch Catch fire again on
                      replay. Worth the turn only when nothing else can attack; otherwise
                      60 damage is a poor trade against a charged attacker's 240.
      Fezandipiti  -- Cruel Arrow does exactly 100 to any of the opponent's Pokemon, so it
                      is a sniper only when a target sits at or below 100 HP, and only when
                      it will not simply be knocked out in reply.
    """
    ...
    can_ko_now = opp_active is not None and _best_usable_damage(mon) >= opp_active.hp
    if can_ko_now:
        return 4000  # taking the prize beats every positional consideration

    if mon.id == OGERPON:
        return -900  # keep it in Tera safety on the Bench

    if mon.id == MEOWTH_EX:
        others_can_attack = any(
            b.id != MEOWTH_EX and _best_usable_damage(b) > 0 for b in bench
        )
        return -600 if others_can_attack else 2500  # escape valve only when the bench is dead

    # Doomed promotion: the opponent's Active can knock out whatever we send up and we
    # cannot take a prize back. Then the promotion is a sacrifice, so send the cheapest
    # body -- no Rule Box (1 prize instead of 2 or 3), a duplicate whose role is already
    # covered on board, and nothing that can currently attack. Never a Pokemon whose
    # Ability is the only one of its kind in play (Meganium's Wild Growth, and the single
    # copies of the support ex) -- losing it turns a chump block into a real setback.
    doomed = (
        opp_active is not None
        and _best_usable_damage(opp_active) >= mon.hp
        and not can_ko_now
    )
    if doomed:
        card = _card_data().get(mon.id)
        rule_box = bool(card and (card.ex or card.megaEx))
        board_ids = [b.id for b in bench] + (
            [me.active[0].id] if me.active and me.active[0] is not None else []
        )
        duplicate = board_ids.count(mon.id) > 1
        idle = _best_usable_damage(mon) <= 0
        sacrifice = 0
        if not rule_box:
            sacrifice += 900          # 1 prize instead of 2 or 3
        if duplicate:
            sacrifice += 600          # its role survives on the other copy
        if idle:
            sacrifice += 400          # it was contributing nothing this turn
        if not duplicate and mon.id in (MEGANIUM_ID, OGERPON, MEOWTH_EX, FEZANDIPITI_EX):
            sacrifice -= 1200         # sole holder of an Ability the deck needs
        return sacrifice

    if mon.id == FEZANDIPITI_EX:
        opp_board = ([opp.active[0]] if opp.active and opp.active[0] is not None else []) + [
            b for b in (opp.bench or []) if b is not None
        ]
        snipe_target = any(t.hp <= CRUEL_ARROW_DAMAGE for t in opp_board)
        survives = opp_active is None or _best_usable_damage(opp_active) < mon.hp
        return 1800 if (snipe_target and survives) else -700
...
def _choose_card(obs: Observation) -> list[int]:
    ...
    if sel.context in opponent_target_contexts:
        ...
        if own_board_indices:
            ...
            opp_active = opp.active[0] if opp.active else None
            # Deck roles apply to promotion only. ATTACH_FROM (the Solar Transfer
            # destination) also lands in this branch and must keep the generic ranking.
            promoting = sel.context in (SelectContext.TO_ACTIVE, SelectContext.SWITCH)

            def _own_board_key(i):
                mon = _option_pokemon(options[i], obs.current)
                score = _live_attacker_score(mon, opp_active)
                if promoting:
                    score += _promotion_adjust(mon, obs.current, opp_active)
                return score
