...
def _attacker_concentration_bonus(mon) -> int:
    """Cancel the prize-liability penalty for an ALREADY attack-ready rule-box attacker.

    `_live_attacker_score` subtracts `120 * prize_value`, which on this deck is -360 for
    Mega Venusaur ex and -240 for Teal Mask Ogerpon ex / Meowth ex / Fezandipiti ex. That
    penalty is right for "don't walk an ex into the Active slot to be sniped", but it is
    wrong for the two choices that actually use this score here:

      * forced promotion after a knockout -- it pushes a cheap support chump into the
        Active slot instead of the attacker that can immediately take a prize back;
      * the Solar Transfer / Energy Switch destination (`SelectContext.ATTACH_FROM`, 53
        selections per 10 games on this deck) -- it routes Energy AWAY from Mega Venusaur
        ex, the exact Pokemon the energy engine exists to charge.

    Gated on `usable_damage > 0` so it stays silent during setup, when nothing is powered
    yet and generic live-attacker scoring is what we want. The `dmg * 5` term breaks ties
    toward the harder hitter.

    Prior evidence: identical mechanism screened 121-79 (60.5%, z=2.97, p=0.00298, zero
    faults) on the Claude_Grass_Venusaur deck and was closed for portfolio reasons rather
    than on evidence -- see sub-agents/human-review/rejected/Claude_Grass_Venusaur-EXP-0002.
    """
    if mon is None:
        return 0
