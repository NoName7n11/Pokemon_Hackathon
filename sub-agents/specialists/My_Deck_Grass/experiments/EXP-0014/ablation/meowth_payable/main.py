...
def _promotion_adjust(mon, state, opp_active) -> int:
    """Ablation: Meowth ex Tuck Tail escape when nothing else on the bench can attack."""
    if mon is None or state is None or mon.id != 1071:
        return 0
    ...
    # THE MISSING PRECONDITION. The trigger "nothing else can attack" almost always means
    # there is no Energy on board -- in which case Meowth ex cannot pay Tuck Tail's 3
    # Colorless either. It then just sits in the Active spot and dies for 2 prizes. The
    # isolated rule without this guard measured 73-127 (36.5%, p=0.0001). Promote it as an
    # escape valve only when it can ACTUALLY attack right now.
    if alternative or _best_usable_damage(mon) <= 0:
        return 0
    return 200
