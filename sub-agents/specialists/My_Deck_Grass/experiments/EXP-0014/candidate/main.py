...
OGERPON, MEOWTH_EX, FEZANDIPITI_EX, MEGANIUM_ID = 96, 1071, 140, 710
...
def _promotion_adjust(mon, state, opp_active) -> int:
    ...
    Every positional rule below is CONDITIONAL on a real alternative existing. EXP-0013
    applied them unconditionally and scored 32.5%: a charged 210 HP Ogerpon ex ranked
    below a 70 HP Chikorita, so the agent chump-blocked while holding a live attacker.
    A positional preference must never outrank "this is the only thing that can fight".
    """
    ...
    if opp_active is not None and _best_usable_damage(mon) >= opp_active.hp:
        return 4000                                   # take the prize, always

    # Is there any OTHER candidate that can attack at all? Without one, no positional
    # rule may fire -- that is the defect that sank EXP-0013.
    alternative = any(b is not mon and _best_usable_damage(b) > 0 for b in bench)
    incoming = _best_usable_damage(opp_active) if opp_active is not None else 0
    # Situation-level, evaluated once: does everything we could send up simply die AND
    # can none of it attack back? Sacrificing is only right when there is no damage to
    # trade -- promoting an attacker that dies after dealing 240 beats chumping for 0.
    all_doomed = (
        bool(bench)
        and incoming > 0
        and all(incoming >= b.hp for b in bench)
        and not any(_best_usable_damage(b) > 0 for b in bench)
    )
    ...
    if all_doomed:
        ...
        sacrifice = 0
        if not rule_box:
            sacrifice += 900                          # 1 prize, not 2 or 3
        if duplicate:
            sacrifice += 600                          # role survives on the other copy
        if _best_usable_damage(mon) <= 0:
            sacrifice += 400                          # contributing nothing anyway
        if not duplicate and mon.id in (MEGANIUM_ID, OGERPON, MEOWTH_EX, FEZANDIPITI_EX):
            sacrifice -= 1200                         # sole holder of a needed Ability
        return sacrifice

    if mon.id == OGERPON:
        return -900 if alternative else 0             # Tera immunity is worth keeping
    if mon.id == MEOWTH_EX:
        # Tuck Tail deals 60 and returns it plus attached cards to hand: dodges the
        # 2-prize liability and re-enables Last-Ditch Catch. Only when nothing else fights.
        return -600 if alternative else 2500
    if mon.id == FEZANDIPITI_EX:
        ...
        snipe = any(t.hp <= CRUEL_ARROW_DAMAGE for t in opp_board)
        survives = incoming < mon.hp
        if snipe and survives:
            return 1800
        return -700 if alternative else 0
    return 0
