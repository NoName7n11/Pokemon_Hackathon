...
def _conditional_ability_index(options, indices, me, state) -> int:
    """Order abilities by whether they ADD a resource or merely MOVE one -- conditionally.

    EXP-0007 tested a BLANKET reorder (Teal Dance always ahead of Solar Transfer) and lost
    at 45.0%. The conditional form is different: Teal Dance pulls a NEW Basic {G} out of
    hand and draws a card, so it is the better use of the turn UNLESS the Active cannot
    currently attack -- in which case routing Energy to it with Solar Transfer is what
    actually converts the turn into damage. Blanket ordering starves Solar Transfer in the
    exact spots where it decides the game; this only defers it when nothing needs routing.

    Teal Dance is also gated on actually holding a Basic {G}: with an empty hand it is a
    no-op that would burn a slot of the per-turn ability cap.
    """
    cards = _card_data()
    my_active = me.active[0] if me.active and me.active[0] is not None else None
    active_can_attack = _best_usable_damage(my_active) > 0 if my_active is not None else False
    hand_has_energy = any(
        (cards.get(getattr(c, "id", None)) or _NO_CARD).cardType == CardType.BASIC_ENERGY
    ...
    def score(i):
        opt = options[i]
        card_id = opt.cardId
        if not card_id and opt.index is not None and opt.index >= 0:
            if opt.area == AreaType.ACTIVE:
                card_id = my_active.id if my_active is not None else None
            elif opt.area == AreaType.BENCH:
                bench = me.bench or []
                card_id = bench[opt.index].id if opt.index < len(bench) and bench[opt.index] else None
        if card_id == 140:      # Fezandipiti ex, Flip the Script: draw 3, pure gain
            return 400
        if card_id == 1071:     # Meowth ex, Last-Ditch Catch: tutor a Supporter
            return 300
        if card_id == 96:       # Teal Mask Ogerpon ex, Teal Dance: adds Energy from hand
            return 250 if hand_has_energy else 40
        if card_id == 652:      # Mega Venusaur ex, Solar Transfer: only moves Energy
            return 100 if active_can_attack else 350
        return 0

    return max(indices, key=lambda i: (score(i), -i))
...
def _choose_main(obs: Observation) -> list[int]:
    ...
    if OptionType.ABILITY in by_type and not _ability_cap_reached(state):
        return [_conditional_ability_index(options, by_type[OptionType.ABILITY], me, state)]
