...
def _choose_yes_no(obs: Observation) -> list[int]:
    ...
    Exception: IS_FIRST. The baseline answers "yes, go first" every single game and that
    choice was never evaluated -- it is one binary decision per game, so it is invisible
    in any per-decision census yet it conditions the entire game. This arm answers NO to
    isolate whether going second is better on this deck.
    """
    sel = obs.select
    assert sel is not None
