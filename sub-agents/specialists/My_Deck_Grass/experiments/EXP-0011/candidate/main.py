...
_MCTS_AGENT = None
_MCTS_UNAVAILABLE = False
MCTS_CONFIG = "mcts_selective_budget.json"


def _mcts_agent():
    """Lazily build plan_1's bounded UCT searcher, or None if it cannot be had.

    The heuristic in this file is handed to the search as fallback policy, rollout policy
    AND seed candidate at every node, so the accepted EXP-0005 energy routing feeds the
    search three separate ways rather than being replaced by it.

    Built lazily because the benchmark harnesses assign `_MY_DECK` after importing this
    module -- constructing at import time would bind the wrong deck. Any failure disables
    the search permanently for this process and leaves the existing 1-ply lookahead in
    place, so a missing plan_1 tree degrades to the accepted v001 agent rather than
    forfeiting.
    """
    global _MCTS_AGENT, _MCTS_UNAVAILABLE
    if _MCTS_UNAVAILABLE or _MCTS_AGENT is not None:
        return _MCTS_AGENT
    try:
        import sys as _sys
        ...
        deck = _my_deck_ids()
        if len(deck) != 60:
            raise ValueError(f"deck has {len(deck)} cards, need 60")
        # A config sitting next to this module wins, so a hyperparameter sweep can ship
        ...
        _MCTS_AGENT = Plan1MCTSAgent(
            ...
            opponent_deck=deck,
            fallback_policy=_greedy_select,
            rollout_policy=_greedy_select,
            action_observer=_record_if_ability,
        )
    except Exception:
        _MCTS_UNAVAILABLE = True
        _MCTS_AGENT = None
    return _MCTS_AGENT


def _agent_impl(obs_dict: dict) -> list[int]:
    """Pokémon TCG agent: bounded UCT search over MAIN decisions, falling back to the
    1-ply lookahead and then to greedy on any failure.
    ...
    if SEARCH_MAIN and obs.select.type == SelectType.MAIN and obs.current is not None:
        # The ability cap MUST gate the search path too. Plan1MCTSAgent enumerates legal
        # options straight from the engine and never sees ABILITY_CAP_PER_TURN, so the
        # unbounded-Solar-Transfer loop documented above reappears through it: a 10-game
        # trace of the ungated version picked ABILITY 1092 times and END 18 times, against
        # 52 and 958 for the heuristic, and games ran 344 steps instead of 154. The eval's
        # `energies * 5` term makes "move Energy again" score highest forever, exactly as
        # _ability_cap_reached's docstring describes. Once the cap is hit, hand the decision
        # back to the heuristic ladder, which filters capped ABILITY options out.
        searcher = None if _ability_cap_reached(obs.current) else _mcts_agent()
        if searcher is not None:
            try:
                chosen = _clamp(searcher.act(obs_dict), obs.select, len(obs.select.option))
                if chosen:
                    # act() already reported the action through action_observer.
                    return chosen
            except Exception:
                pass  # fall through to the 1-ply lookahead below
