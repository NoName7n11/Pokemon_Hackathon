...
def _attach_target_score(target, is_active: bool) -> int:
    """Score one manual-attachment destination by whether the Energy will ever be used.

    The old rule was `Active gets 10000, Bench gets hp`, i.e. the once-per-turn manual
    attachment always went to whatever happened to be Active. Measured over 10 games:
    177 of 188 ATTACH-offering MAIN selections (94%) offered more than one target, and
    22 of 93 attachments landed on Meowth ex or Fezandipiti ex -- support Pokemon whose
    Colorless attacks this deck never uses. Energy put there is dead until Solar Transfer
    happens to rescue it, which is why fixing the router (EXP-0005) was worth +7 points.

    What matters is progress toward an attack: an attacker one Energy short of its cost
    is the best destination, an attacker already able to attack is next, and a Pokemon
    that cannot meaningfully attack at all is last regardless of its HP.
    """
    if target is None:
        return -10**6
    card = _card_data().get(target.id)
    if card is None:
        return 0

    attacks = _attack_data()
    have = list(target.energies)
    boosted = have + [EnergyType.GRASS]
    best_now = 0
    best_after = 0
    for aid in card.attacks:
        ...
        if atk is None:
            continue
        if _can_pay(atk.energies, have):
            best_now = max(best_now, atk.damage)
        if _can_pay(atk.energies, boosted):
            best_after = max(best_after, atk.damage)

    # Scale by the damage actually unlocked, not a flat bonus: unlocking Bulbasaur's
    # 10-damage Bind Down must not outrank charging a 240-damage Jungle Dump.
    score = 0
    if best_after > best_now:
        score += 2000 + best_after * 20  # this attachment unlocks a stronger attack
    elif best_now > 0:
        score += 1000 + best_now * 5     # already online; more Energy is only insurance
    else:
        score += 200                     # cannot attack even with the Energy
    if is_active:
        score += 800                     # Energy on the Active is usable this turn
    return score + target.hp // 10


def _best_attach_index(options, indices, me):
    """Send the manual attachment to whoever it brings closest to attacking."""
    best_i, best_score = indices[0], -(10**9)
    for i in indices:
        opt = options[i]
        target = None
        is_active = opt.inPlayArea == AreaType.ACTIVE
        if is_active:
            target = me.active[0] if me.active and me.active[0] is not None else None
        ...
        score = _attach_target_score(target, is_active)
        if score > best_score:
