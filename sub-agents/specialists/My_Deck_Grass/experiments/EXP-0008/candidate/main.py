...
ABILITY_CAP_PER_TURN = 8    # see _ability_cap_reached: guards against unbounded Abilities.
                            # Raised 4 -> 8 for this deck. The cap is a LOOP guard, not a
                            # strategic limit, and 4 was tuned elsewhere. This deck runs
                            # three abilities that all want to fire on the same turn --
                            # Solar Transfer (unlimited), Teal Dance and Flip the Script --
                            # and EXP-0007 measured that reallocating the 4-use budget away
                            # from Solar Transfer LOSES (45.0%), because correct Solar
                            # Transfer routing is what produced the accepted +7 points.
                            # So add budget instead of re-dividing it. The loop this guards
                            # against is self-detecting: it shows up as timeouts or a
                            # step-count blowout in the benchmark, both of which are
                            # reported per run.
