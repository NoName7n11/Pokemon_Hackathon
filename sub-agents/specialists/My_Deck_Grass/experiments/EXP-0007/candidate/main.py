...
# Ability ordering. ABILITY_CAP_PER_TURN bounds how many Abilities fire per turn, so the
# order is a real allocation decision, not a cosmetic one. The rule: abilities that ADD a
# resource once per turn go before the one that merely MOVES resources without limit.
#
#   Flip the Script  (Fezandipiti ex) draw 3, once per turn, only legal after a knockout
#   Last-Ditch Catch (Meowth ex)      tutor a Supporter, once, on being benched
#   Teal Dance       (Ogerpon ex)     attach a Basic {G} from HAND, then draw; once per turn
#   Solar Transfer   (Mega Venusaur)  move a Basic {G} already in play; UNLIMITED
#
# Solar Transfer last is the important part: running it first spends the cap moving Energy
# around a board that Teal Dance was about to add to, and Teal Dance's draw can change what
# the right routing even is. Measured over 10 games, the old by_type[ABILITY][0] rule took
# Solar Transfer 118 times to Teal Dance's 64 purely because Mega Venusaur ex is listed
# first, on the 111 turns where both were offered.
_ABILITY_PRIORITY = {
    140: 400,   # Fezandipiti ex  -- Flip the Script
    1071: 300,  # Meowth ex       -- Last-Ditch Catch
    96: 200,    # Ogerpon ex      -- Teal Dance
    652: 100,   # Mega Venusaur   -- Solar Transfer (unlimited; run it last)
}


def _ability_option_score(opt, me) -> int:
    """Priority of one ABILITY option, resolved through its area+index."""
    card_id = opt.cardId
    if not card_id:
        index = opt.index
        ...
        if opt.area == AreaType.ACTIVE:
            ...
            card_id = mon.id if mon is not None else None
        elif opt.area == AreaType.BENCH:
            bench = me.bench or []
            card_id = bench[index].id if index < len(bench) and bench[index] is not None else None
        elif opt.area == AreaType.HAND:
            hand = getattr(me, "hand", None) or []
            card_id = getattr(hand[index], "id", None) if index < len(hand) else None
    if not card_id:
        return 0
    return _ABILITY_PRIORITY.get(card_id, 0)


def _best_ability_index(options, indices, me) -> int:
    """Highest-priority ability; ties keep engine order so the change stays minimal."""
    return max(indices, key=lambda i: (_ability_option_score(options[i], me), -i))
...
def _choose_main(obs: Observation) -> list[int]:
    ...
    if OptionType.ABILITY in by_type and not _ability_cap_reached(state):
        return [_best_ability_index(options, by_type[OptionType.ABILITY], me)]
