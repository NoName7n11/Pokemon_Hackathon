...
WIN_SCORE = 1e9
CHARIZARD_EFFECT_FALLBACK_DAMAGE = 240
...
def _best_attack_index(options, indices, attacker=None):
    ...
    card = _card_data().get(attacker.id) if attacker is not None else _active_card_from_options(options)
    best_i, best_dmg = indices[0], -1
    for i in indices:
        atk = attacks.get(options[i].attackId)
        dmg = _attack_damage_value(atk, attacker, card)
        if dmg > best_dmg:
...
def _text_of(obj) -> str:
    parts = []
    for attr in ("name", "text", "effect", "description"):
...
def _is_mega_charizard(card) -> bool:
    name = str(getattr(card, "name", "")).lower()
    return "mega charizard x ex" in name or "mega charizard y ex" in name


def _energy_count(mon, energy_type=None) -> int:
    if mon is None:
        return 0
    if energy_type is None:
        return len(mon.energies)
    return sum(1 for e in mon.energies if e == energy_type or e == EnergyType.RAINBOW)


def _discard_required(text: str) -> int:
    match = re.search(r"discard (\d+) [a-z ]*energ", text)
    return int(match.group(1)) if match else 0


def _charizard_effect_damage(atk, mon, card) -> int:
    """Estimate zero-printed Mega Charizard X/Y ex attack damage from effect text."""
    if atk is None or card is None or not _is_mega_charizard(card):
        return 0
    text = _text_of(atk).lower()
    attached = _energy_count(mon)
    fire_attached = _energy_count(mon, EnergyType.FIRE)
    required_discard = _discard_required(text)
    if required_discard and attached and attached < required_discard:
        return 0

    match = re.search(r"(\d+)\s+damage for each", text)
    if match:
        per_energy = int(match.group(1))
        count = fire_attached if "fire energ" in text else attached
        if "discard all" in text or "for each" in text:
            return per_energy * max(count, required_discard)

    match = re.search(r"does (\d+) damage", text)
    if match:
        return int(match.group(1))

    if attached and required_discard and attached < required_discard:
        return 0
    return CHARIZARD_EFFECT_FALLBACK_DAMAGE


def _attack_damage_value(atk, mon=None, card=None) -> int:
    if atk is None:
        return 0
    if atk.damage:
        return atk.damage
    return _charizard_effect_damage(atk, mon, card)


def _active_mon_from_options(options):
    for opt in options:
        mon = getattr(opt, "pokemon", None)
        if mon is not None:
            return mon
    return None


def _active_card_from_options(options):
    mon = _active_mon_from_options(options)
    if mon is not None:
        return _card_data().get(mon.id)
    return None
...
def _card_power(card_id):
    ...
    if card is None:
        return 0
    best_damage = max((_attack_damage_value(attacks[a], None, card) for a in card.attacks if a in attacks), default=0)
...
def _best_usable_damage(mon) -> int:
    ...
    for aid in card.attacks:
        ...
        if a is not None and _can_pay(a.energies, mon.energies):
            best = max(best, _attack_damage_value(a, mon, card))
    return best
