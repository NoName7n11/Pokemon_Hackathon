...
def _setup_active_priority(opt, obs):
    state = obs.current
    if state is None or opt.area != 2 or opt.index is None:
        return None
    ...
    if hand is None or opt.index < 0 or opt.index >= len(hand):
        return None
    card = _card_data().get(hand[opt.index].id)
    ...
    priorities = {
        ...
        "budew": 30000,
        "meowth ex": 20000,
        "fezandipiti ex": 10000,
    }
    return priorities.get(str(name).lower(), 0)


def _choose_setup_active_card(obs):
    sel = obs.select
    assert sel is not None
    priorities = []
    for i, opt in enumerate(sel.option):
        priority = _setup_active_priority(opt, obs)
        if priority is None:
            return None
        priorities.append(priority)
    ranked = sorted(range(len(sel.option)), key=lambda i: priorities[i], reverse=True)
...
def _choose_card(obs: Observation) -> list[int]:
    ...
    assert sel is not None
    options = sel.option
    if sel.type == SelectType.CARD and sel.context == SelectContext.SETUP_ACTIVE_POKEMON:
        setup_choice = _choose_setup_active_card(obs)
