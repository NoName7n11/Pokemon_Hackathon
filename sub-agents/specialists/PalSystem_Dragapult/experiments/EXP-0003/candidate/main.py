...
def _card_name(card_id) -> str:
    ...
    return getattr(card, "name", "") if card is not None else ""


def _is_dragapult_line(mon) -> bool:
    name = _card_name(mon.id).lower() if mon is not None else ""
    return "dreepy" in name or "drakloak" in name or "dragapult ex" in name


def _is_munkidori(mon) -> bool:
    return "munkidori" in (_card_name(mon.id).lower() if mon is not None else "")


def _energy_type_from_card(card_id):
    card = _card_data().get(card_id)
    for attr in ("energyType", "energy_type", "type"):
        energy = getattr(card, attr, None)
        if energy in (EnergyType.FIRE, EnergyType.PSYCHIC, EnergyType.DARKNESS):
            return energy
    name = _card_name(card_id).lower()
    if "fire" in name:
        return EnergyType.FIRE
    if "psychic" in name:
        return EnergyType.PSYCHIC
    if "darkness" in name or "dark" in name:
        return EnergyType.DARKNESS
    return None


def _option_energy_type(opt):
    energy = getattr(opt, "energyType", None)
    if energy in (EnergyType.FIRE, EnergyType.PSYCHIC, EnergyType.DARKNESS):
        return energy
    return _energy_type_from_card(opt.cardId) if opt.cardId is not None else None


def _has_energy(mon, energy_type) -> bool:
    return mon is not None and any(e == energy_type or e == EnergyType.RAINBOW for e in mon.energies)


def _missing_phantom_dive_energy(mon):
    missing = []
    if not _has_energy(mon, EnergyType.FIRE):
        missing.append(EnergyType.FIRE)
    if not _has_energy(mon, EnergyType.PSYCHIC):
        missing.append(EnergyType.PSYCHIC)
    return missing


def _munkidori_darkness_live(me, opp) -> bool:
    own = ([me.active[0]] if me.active and me.active[0] else []) + list(me.bench)
    opponents = ([opp.active[0]] if opp.active and opp.active[0] else []) + list(opp.bench)
    has_damaged_source = any(mon.hp < mon.maxHp for mon in own)
    has_target = any(mon.hp > 0 for mon in opponents)
    needs_darkness = any(_is_munkidori(mon) and not _has_energy(mon, EnergyType.DARKNESS) for mon in own)
    return has_damaged_source and has_target and needs_darkness


def _dragapult_attach_index(options, indices, me, opp):
    """Deck-specific Energy routing; return None when generic attach should apply."""
    best_i, best_score = None, -1
    darkness_live = _munkidori_darkness_live(me, opp)
    for i in indices:
        opt = options[i]
        energy = _option_energy_type(opt)
        if energy is None:
            continue
        target = None
        ...
        score = -1
        if _is_dragapult_line(target) and energy in (EnergyType.FIRE, EnergyType.PSYCHIC):
            missing = _missing_phantom_dive_energy(target)
            if energy in missing:
                completed = len(missing) == 1
                active_bonus = 4000 if opt.inPlayArea == AreaType.ACTIVE else 0
                score = 20000 + active_bonus + (10000 if completed else 0) + len(target.energies) * 500 + target.hp
        elif darkness_live and _is_munkidori(target) and energy == EnergyType.DARKNESS:
            score = 18000 + target.hp
...
def _energy_card_choice(obs: Observation, indices):
    """Prefer Crispin-style Fire/Psychic access, preserving generic choices otherwise."""
    ...
    dragapult_missing = []
    for mon in own:
        if _is_dragapult_line(mon):
            dragapult_missing.extend(_missing_phantom_dive_energy(mon))
    wants = []
    for energy in (EnergyType.FIRE, EnergyType.PSYCHIC):
        if energy in dragapult_missing:
            wants.append(energy)
    if _munkidori_darkness_live(me, opp):
        wants.append(EnergyType.DARKNESS)
    if not wants:
        return None
    ...
    for energy in wants:
        for i in indices:
            if i not in chosen and _option_energy_type(obs.select.option[i]) == energy:
                chosen.append(i)
                break
...
def _best_attach_index(options, indices, me, opp=None):
    """Prefer powering the Active, then the strongest Bench target."""
    if opp is not None:
        routed = _dragapult_attach_index(options, indices, me, opp)
...
def _choose_main(obs: Observation) -> list[int]:
    ...
    if OptionType.ATTACH in by_type and not state.energyAttached:
        return [_best_attach_index(options, by_type[OptionType.ATTACH], me, opp)]
...
def _choose_card(obs: Observation) -> list[int]:
    ...
    if sel.context not in weakest_first_contexts and sel.context not in opponent_target_contexts:
        energy_choice = _energy_card_choice(obs, indices)
        if energy_choice is not None:
            return energy_choice[: sel.maxCount] if sel.maxCount > 0 else []
