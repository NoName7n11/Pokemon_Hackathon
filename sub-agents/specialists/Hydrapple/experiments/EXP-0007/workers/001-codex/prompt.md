...
When a normal Energy attachment makes an attack usable this turn, apply that readiness preference only to the Active Pokemon; never give the readiness bonus to a Benched target, and preserve the shipped attachment ranking otherwise.

Expected effect:
Reduce wasted Active attachments without recreating the measured harmful bench-first Energy routing.
