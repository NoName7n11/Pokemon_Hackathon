...
def _option_energy_type(opt):
    """Best-effort Energy type for an attach option's hand card."""
    ...
    if card is None:
        return None
    for attr in ("energyType", "energy_type", "energy", "energies"):
        energy_type = getattr(card, attr, None)
        if energy_type is None:
            continue
        if isinstance(energy_type, (list, tuple)):
            if len(energy_type) == 1:
                return energy_type[0]
            return EnergyType.RAINBOW
        return energy_type
    return None
...
def _best_attach_index(options, indices, me):
    ...
    for i in indices:
        ...
        if opt.inPlayArea == AreaType.ACTIVE:
            ...
            energy_type = _option_energy_type(opt)
            if _attack_readiness_after_attachment(target, energy_type):
                score += 500
...
def _has_usable_attack(mon, energies=None) -> bool:
    """Whether this Pokemon has any attack whose cost can be paid."""
    if mon is None:
        return False
    card = _card_data().get(mon.id)
    if card is None:
        return False
    have = mon.energies if energies is None else energies
    attacks = _attack_data()
    for aid in card.attacks:
        attack = attacks.get(aid)
        if attack is not None and _can_pay(attack.energies, have):
            return True
    return False


def _attack_readiness_after_attachment(mon, energy_type) -> bool:
    """True only when this one Energy newly makes an attack usable."""
    if mon is None or energy_type is None:
        return False
    current = list(mon.energies)
    return not _has_usable_attack(mon, current) and _has_usable_attack(mon, current + [energy_type])
