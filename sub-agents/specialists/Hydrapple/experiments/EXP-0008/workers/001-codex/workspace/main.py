...
def _best_attach_index(options, indices, me):
    ...
    for i in indices:
        ...
        if opt.inPlayArea == AreaType.ACTIVE:
            ...
            if _attack_unlocked_by_attach(target, opt.cardId):
                score += 20000
...
def _energy_types_from_card(card_id):
    """Best-effort Basic/Special Energy typing from cg card metadata."""
    card = _card_data().get(card_id)
    if card is None or getattr(card, "cardType", None) != CardType.ENERGY:
        return []
    for attr in ("energyType", "energyTypes", "basicEnergyType", "providesEnergyType", "providesEnergyTypes"):
        energy_types = getattr(card, attr, None)
        if energy_types is None:
            continue
        if isinstance(energy_types, (list, tuple, set)):
            return list(energy_types)
        return [energy_types]
    return []


def _has_usable_attack(card_id, energies) -> bool:
    card = _card_data().get(card_id)
    if card is None:
        return False
    attacks = _attack_data()
    for aid in card.attacks:
        ...
        if attack is not None and _can_pay(attack.energies, energies):
            return True
    return False


def _attack_unlocked_by_attach(mon, card_id) -> bool:
    if mon is None:
        return False
    energy_types = _energy_types_from_card(card_id)
    if not energy_types:
        return False
    return (
        not _has_usable_attack(mon.id, mon.energies)
        and _has_usable_attack(mon.id, list(mon.energies) + energy_types)
    )
