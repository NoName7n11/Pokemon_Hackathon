...
def _attached_energy_type(card_id):
    """Best-effort Basic Energy type lookup for an Energy card in hand."""
    card = _card_data().get(card_id)
    energy_card_type = getattr(CardType, "ENERGY", None)
    if card is None or (energy_card_type is not None and card.cardType != energy_card_type):
        return None
    for attr in ("energyType", "energy_type"):
        energy_type = getattr(card, attr, None)
        if energy_type is not None:
            return energy_type
    name = getattr(card, "name", "")
    energy_types = []
    for type_name in ("GRASS", "FIRE", "WATER", "LIGHTNING", "PSYCHIC", "FIGHTING", "DARKNESS", "METAL", "FAIRY"):
        energy_type = getattr(EnergyType, type_name, None)
        if energy_type is not None:
            energy_types.append(energy_type)
    for energy_type in energy_types:
        if energy_type.name.title() in name:
            return energy_type
    return None


def _attachment_unlocks_attack(mon, energy_type) -> bool:
    """True when one attached Energy changes this Pokemon from unable to able."""
    if mon is None or energy_type is None:
        return False
    card = _card_data().get(mon.id)
    if card is None:
        return False
    attacks = _attack_data()
    have = list(mon.energies)
    for aid in card.attacks:
        attack = attacks.get(aid)
        if attack is None:
            continue
        if not _can_pay(attack.energies, have) and _can_pay(attack.energies, have + [energy_type]):
            return True
    return False


def _best_attach_index(options, indices, me):
    """Prefer attachments that unlock an attack, then Active, then strongest target."""
    best_i, best_score = indices[0], -1
    for i in indices:
        opt = options[i]
        score = 0
        energy_type = _attached_energy_type(opt.cardId)
        if opt.inPlayArea == AreaType.ACTIVE:
            ...
            target = None
        if _attachment_unlocks_attack(target, energy_type):
            score += 100000
        if score > best_score:
